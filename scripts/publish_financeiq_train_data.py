...
def parse_args() -> argparse.Namespace:
    """Parse CLI args for publishing final FinanceIQ train_data files."""
    parser = argparse.ArgumentParser(description="Publish final FinanceIQ filtered data into train_data.")
    parser.add_argument(
        "--round1_keep",
        nargs=2,
        required=True,
        metavar=("TRAIN_KEEP", "VAL_KEEP"),
    )
    parser.add_argument(
        "--round1_drop",
        nargs=2,
        required=True,
        metavar=("TRAIN_DROP", "VAL_DROP"),
    )
    parser.add_argument(
        "--round2_keep",
        nargs=2,
    ...
    parser.add_argument("--output_dir", default=str(REPO_ROOT / "train_data" / "financeiq"))
    return parser.parse_args()


def write_copy(src: Path, dst: Path) -> int:
    """Copy a JSONL file while counting rows."""
    ensure_parent(dst)
    count = 0
...
def main() -> int:
    """Publish final keep/drop artifacts into train_data/financeiq."""
    args = parse_args()
